...
def load_data(args):
    ...
    if args.dataset == 'unsafes':
        ...
        for question in all_questions:
            instr_resp = question['instr-resp'][0]
            if 'instruction' in instr_resp:
                question['question'] = instr_resp['instruction']
            elif 'unsafe_instruction' in instr_resp:
                question['question'] = instr_resp['unsafe_instruction']
            else:
                raise KeyError(f"Neither 'instruction' nor 'unsafe_instruction' found in instr-resp for question {question['id']}")
    else:
    ...
    if max_questions is not None and max_questions > 0 and max_questions < len(all_questions):
